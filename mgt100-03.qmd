...
- Hence for every $j\ne 1$, divide $s_{jt}$ by $s_{1t}$; the denominators cancel:

$$\frac{s_{jt}}{s_{1t}}=\frac{e^{x_{jt}\beta-\alpha p_{jt}} / \sum_k e^{x_{kt}\beta-\alpha p_{kt}}}{1 / \sum_k e^{x_{kt}\beta-\alpha p_{kt}}}=e^{x_{jt}\beta-\alpha p_{jt}}$$

- Taking logs of both sides and adding an error term gives a linear regression form of the model:
...
If $\alpha=-1$ and $\beta=1$: $\quad V_A=5-p_A$; $\quad V_B=4-2=2$; $\quad V_C=0$ (normalization)

- $P_{1A}=e^{1}/(e^{1}+e^{2}+e^{0})=0.24$; $\quad P_{1B}=e^{2}/(e^{1}+e^{2}+e^{0})=0.67$
- $P_{2A}=e^{3}/(e^{3}+e^{2}+e^{0})=0.71$; $\quad P_{2B}=e^{2}/(e^{3}+e^{2}+e^{0})=0.26$

::: {.callout-note appearance="minimal"}
Person 1 chose B and Person 2 chose A. Product C is the "outside option" — not purchasing. What happens to the choice probabilities if you change $\alpha$ to $-1.1$? What if you change $\alpha$ to $-0.9$?
:::
...
3. Likelihood function is globally concave in $\alpha$ and $\beta$, ensuring fast and reliable estimation
...
1. Assuming $\epsilon_{ijt}\sim$*i.i.d.*$EV_1(0,1)$ is algebraically convenient but unrealistic
...
3. Market share derivatives depend on market shares alone (IIA; see Train Sec. 3.6)
    - We'd expect more-differentiated products to face less-elastic demand, holding shares constant, but IIA does not allow this
4. Price Endogeneity
