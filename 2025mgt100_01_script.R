...
# You don't need to reinstall or update again this quarter.
...
### 2. The discount variable indicates which, if any, phone was on sale when the customer bought a phone.
###    Sometimes, no phone was on discount at the time a customer made their purchase.  
###    In these cases, the discount variable has a value of NA to indicate "not available" (ie, missing data).  
###    For how many customers was there no phone on discount?  Use the is.na() function to help you with the count.    

### 3. Calculate the number of customers that bought a phone that was on discount 
###    (ie, discount and phone_id have the same value).  
###    Use the double-equals operator (==) to test if things are equal.

### 4. Visualize the relationship between price and screen_size for Apple phones.    
###    Visualize the relationship between price and screen_size for Huawei phones.
###    Visualize the relationship between price and screen_size for Samsung phones.    
###    How would you describe the relationship between price and screen size across brands?

### 5. Which brand offers a larger screen size around the 900$ price point?    
