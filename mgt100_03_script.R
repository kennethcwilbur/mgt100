...
# In order to estimate our Multinomial Logit (MNL) model, we need to know both 
...
# selected options to non-selected options. We'll need to wrangle data to
# facilitate those comparisons for every choice occasion

# import customer data
cust_dat <- read_csv("2025mgt100-data/smartphone_customer_data.csv", show_col_types = F)
n <- nrow(cust_dat)

# replace missing 'discount' values (currently NA) with empty string ("")
#    this is because we know that discounts were measured when absent
#    Always beware confusing zeros and NAs
...
# We'll need customer segments to estimate heterogeneous demand models soon, 
#    so we'll identify those now
# Last time we estimated segment memberships in an unrealistic 2-d space 
#    of (gaming, handsize). We did that so we could draw pictures and visualize 
#    projections downward into 1-d. 
# This time we'll use all 6 usage variables, as they more likely indicate
#    customer needs, so they are more relevant to segmentation
set.seed(1357)   # this sets random kmeans starting points to be deterministic

# subk contains the usage variables we base our segmentation on
subk <- cust_dat |> select(gaming, chat, maps, video, social, reading)

# As before, we'll standardize the usage variables, to prevent scale differences
#    between variables from having undue influence over customer segments
sclsubk <- subk |>
  scale() |>
  as_tibble()

# As before we'll draw the elbow graph to choose a number of segments
res <- vector(length = 10)  # have to pre-allocate since we refer to it with index

for (i in 1:10) {
  # run k means
  out <- kmeans(sclsubk, centers = i)
...
ggplot(data.frame(x = 1:10, y = res), aes(x, y)) +
  ...
  theme_minimal()
# Where do we start to see rapidly diminishing returns?

# outk contains our chosen kmeans results, including segment assignments
outk <- kmeans(sclsubk, centers = 3, nstart = 10)
# how big is each segment?
table(outk$cluster)
cust_dat$segment <- factor(outk$cluster) # record segment assignments for each customer in cust_dat
rm(subk, sclsubk, out, outk, res, i)  # let's clean our desk

# import phone attributes
phone_dat <- read_csv("2025mgt100-data/phone_dat.csv", show_col_types = F)


# create dataset for mnl

# Now we need to combine these datasets.  Our customer data has "n" rows and for
# each customer, there were 6 phones available on the market in the year they
# purchased a phone.  So we will construct a dataset that has n*6 rows. This 
...
# In general, R requires you to preallocate a variable when you loop over its 
#    entries ; but it is typically a good practice
...
# Fit a basic MNL model: brand-intercepts only

# Always start simple. For our first model, we will fit a model where our "X"
# variables are just the binary dummy variables that indicate brand. We omit one 
# brand intercept and normalize to identify utility for the remaining 
# options. We omit the intercept by including the bar-zero ("|0") in the formula:

out1 <- mlogit(choice ~ apple + samsung | 0, data = mdat1)

# Congratulations, you just estimated an econometric model!
# Technically, this was not a demand model, since it did not include price (yet)
...
# coefficients inform of us of the impact on estimated market shares: larger positive
...
# Let's add the price variable to make it a demand model:
...
# Third, p-values are smaller, indicating parameter estimates are more precise
...
# That's because we have included a product-level attribute (price) in the model
...
# adding much to the model.  Perhaps this is because screensize and price are
...
# indicates that screensize is an (unobserved in the prior model) attribute that
...
# Instead it included both the true price effect and the missing screensize effect,
# since screensize correlates with price: bigger phones usually cost more. 
# Controlling for screensize changed the price coefficient because it 
# enabled the model to distinguish the missing screensize effect from the price 
# effect

# Recall our market mapping exercis. Samsung's large phone had a very big screen, 
# and a low market share. Maybe we should estimate phone-specific intercepts 
# rather than specifying a common screensize parameter that applies to all phones
...
# (Product dummy variables subsume screensize since every product has only 
#  one possible value for screensize. Screensize does not vary for a given phone, 
# we cannot include it in the model because it would be perfectly collinear with 
# the phone dummies.)
# There will be 5 dummies, as one phone has to be set to 0 for identification.
...
# the screensize variable in the prior model was capturing the average effect
# of screensize across the 3 brands.  Now, we have specified a more flexible model
# in which screensize and all other product-specific differences are accounted
...
## 2. For cohort 1 (using dataset mdat1), fit a multinomial logit model where the 
##    only covariates are the brand intercepts. What is the estimated coefficient
##    for the Huawei brand? Samsung? Apple?

## 3. Now, using mdat1, fit a MNL model with brand intercepts and price. 

## 4. True or false. The brand intercepts got larger in absolute value after 
##    you added price into the model. (I.e, comparing Q2 estimates to Q3 estimates)

## 5. Based on what you've learned in this class, which of the following best explains
##    your answer to Q4?
##     __ You cannot model a demand curve without including a price variable.
##     __ Coefficients always get larger when you add more variables to the model.
##     __ There is a trade-off between model fit and estimate sizes.
##     __ The IIA problem leads the model to overestimate the brand intercepts.
##     __ The brand and price variables are correlated with each other.
##     __ The demand model application requires the future to not be too different from the past.
##     __ The assumption that the errors are independently distributed means that the brand attributes are uncorrelated with prices.

## 6. Fit MNL models using product intercepts and price, using both mdat1 and mdat2.
##    Which cohort of customers was more price sensitive: Those who bought last year,
##    or those who bought two years ago?
...
# dfidx(data, choice, shape, chid.var)    # data prep for mlogit
